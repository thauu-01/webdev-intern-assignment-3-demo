 ...
 * Seeder: seedStudents
 * ─────────────────────
 * Đọc file CSV điểm thi THPT 2024 và import vào MongoDB collection `students`.
 *
 * Đặc điểm:
 *  - Streaming: đọc CSV theo từng dòng, không load toàn bộ vào RAM.
 *  - Batch insert: gom SEED_BATCH_SIZE dòng rồi insertMany một lần.
 *  - Idempotent: dùng ordered:false + bỏ qua lỗi duplicate key (E11000)
 *    → chạy lại nhiều lần không bị lỗi, chỉ skip bản ghi đã tồn tại.
 *  - Progress: in tiến độ mỗi 50.000 dòng.
 *
 * Chạy: npm run seed
 */
...
// ── Config ─────────────────────────────────────────────────────────────────────
// process.cwd() = backend/ (where npm run seed is executed)
// dataset/ is one level above backend/ → ../dataset/
